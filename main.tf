...
resource "aws_route_table" "benchracers_rt" {
  vpc_id = aws_vpc.benchracers_vpc.id
  route { cidr_block = "0.0.0.0/0", gateway_id = aws_internet_gateway.benchracers_igw.id }
...
resource "aws_security_group" "benchracers_ec2_sg" {
  ...
  ingress { from_port = 80, to_port = 80, protocol = "tcp", security_groups = [aws_security_group.benchracers_alb_sg.id] }
